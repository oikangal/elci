...
// eski db.json'larda dizi yoksa garanti altına al
db.data.adjustments ||= [];
...
app.post("/api/login", (req, res) => {
  ...
  if (role === "amb") {
    ...
    const amb = db.data.ambassadors.find(a =>
      (a.username ?? "").toLowerCase() === uname && String(a.pin) === upin
    );
...
app.post("/api/admin/ambassador", async (req, res) => {
  ...
  const exists = db.data.ambassadors.find(
...
// listele (opsiyonel ambassadorId filtresi)
